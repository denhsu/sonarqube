...
export default class Configuration extends React.PureComponent<Props, State> {
  ...
  handleProjectSearch = (query: string) => {
    return searchProjects({
      ...
      ps: 50,
      filter: `query="${query}"`
    }).then(({ components }) => components.map(c => ({ label: c.name, value: c.key })));
  };

  handleProjectSelect = ({ value }: { value: string }) => {
    this.setState(({ settings }) => ({ settings: { ...settings, value } }), this.notifyChange);
  };
